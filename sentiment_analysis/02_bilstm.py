"""
02_bilstm.py
Bi-LSTM 中文情感分析模型 (含 EarlyStopping)
"""
import time
import pandas as pd
import jieba
...
class ChnSentiDataset(Dataset):
    def __init__(self, df, vocab, max_len):
        ...
        self.texts = torch.tensor([
            [vocab.get(w, 1) for w in list(jieba.cut(str(t)))][:max_len]
            + [0] * max(0, max_len - len(list(jieba.cut(str(t)))))
            for t in df['text']
        ], dtype=torch.long)
...
if __name__ == "__main__":
    ...
    dev_df = pd.read_parquet(CONFIG["dev_path"])
    test_df = pd.read_parquet(CONFIG["test_path"])

    counter = Counter()
    for text in train_df['text']:
        counter.update(jieba.cut(str(text)))
    vocab = {w: i + 2 for i, (w, _) in enumerate(counter.most_common(CONFIG["vocab_size"]))}
    vocab['<PAD>'], vocab['<UNK>'] = 0, 1

    train_loader = DataLoader(ChnSentiDataset(train_df, vocab, CONFIG["max_length"]),
                              batch_size=CONFIG["batch_size"], shuffle=True)
    dev_loader = DataLoader(ChnSentiDataset(dev_df, vocab, CONFIG["max_length"]),
                            batch_size=CONFIG["batch_size"])
    test_loader = DataLoader(ChnSentiDataset(test_df, vocab, CONFIG["max_length"]),
                             batch_size=CONFIG["batch_size"])
